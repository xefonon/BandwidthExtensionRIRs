...
class ResidualConv1dGLU(nn.Module):
    ...
    def __init__(self, residual_channels, gate_channels, kernel_size,
                 ...
                 bias=True, use_spectral_norm = False, *args, **kwargs):
        super(ResidualConv1dGLU, self).__init__()
        if use_spectral_norm == False:
            norm_f = weight_norm
        elif use_spectral_norm is None:
            norm_f = nn.Identity()
        else: norm_f = spectral_norm
        ...
        self.conv = norm_f(nn.Conv1d(residual_channels, gate_channels, kernel_size,
                              padding=padding, dilation=dilation,
                              bias=bias, *args, **kwargs))

        # conv output is split into two groups
        gate_out_channels = gate_channels // 2
        self.conv1x1_out = norm_f(nn.Conv1d(gate_out_channels, residual_channels, 1, bias=bias))
        self.conv1x1_skip = norm_f(nn.Conv1d(gate_out_channels, skip_out_channels, 1, bias=bias))
...
class Generator(nn.Module):
    def __init__(self, in_channels, out_channels=1, bias=False,
                 ...
                 postnet_layers=12, postnet_filts=32, use_batch_norm=False, use_spectral_norm = False, postnet_dropout_rate=0.5):
        super().__init__()
        if use_spectral_norm == False:
            norm_f = weight_norm
        elif use_spectral_norm is None:
        ...
        self.first_conv = norm_f(nn.Conv1d(in_channels, residual_channels, 3, padding=1, bias=bias))
        ...
        self.last_conv_layers = nn.Sequential(
            nn.ReLU(True),
            norm_f(nn.Conv1d(skip_out_channels, skip_out_channels, 1, bias=True)),
            nn.ReLU(True),
            norm_f(nn.Conv1d(skip_out_channels, out_channels, 1, bias=True)),
        )
