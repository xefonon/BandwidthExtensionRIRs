...
class Generator(nn.Module):
    ...
    def __init__(self, kernel_size=32, upsamp_ratio=2):
        super().__init__()
        ...
        self.g_dec_depths = [1024, 512, 512, 256, 256, 128, 128, 64, 64, 32, None]
        ...
        for ii, d in enumerate(self.g_dec_depths):
            if ii == 0:
                setattr(self, decvarname + f'{len(self.g_enc_depths) - ii - 1}',
                        spectral_norm(nn.ConvTranspose1d(in_channels=2048,
                                                         out_channels=512,
                                                         kernel_size=self.kernel_size,
                                                         stride=self.upsamp_ratio,
                ...
                setattr(self, decvarname + f'{len(self.g_enc_depths) - ii - 1}',
                        spectral_norm(nn.ConvTranspose1d(self.g_dec_depths[ii - 1],
                                                         d // 2, self.kernel_size, self.upsamp_ratio, self.padding)))
                setattr(self, decvarname + f'{len(self.g_enc_depths) - ii - 1}_nl_attention',
                        Self_Attn(self.g_dec_depths[ii]))
                setattr(self, decvarname + f'{len(self.g_enc_depths) - ii - 1}_nl', nn.PReLU())
            else:
                setattr(self, 'dec_final', spectral_norm(nn.ConvTranspose1d(32, 1,
                                                                            self.kernel_size,
                                                                            self.upsamp_ratio,
                                                                            self.padding)))
    ...
    def forward(self, x, z):
        ...
        d10_c, att_10 = self.dec10_nl_attention(d10_c)
        d9 = self.dec9(d10_c)
        d9_c = self.dec9_nl(torch.cat((d9, e9), dim=1))
        d9_c, att_9 = self.dec9_nl_attention(d9_c)
        d8 = self.dec8(d9_c)
        d8_c = self.dec8_nl(torch.cat((d8, e8), dim=1))
        ic(self.dec8_nl_attention)

        d8_c, att_8 = self.dec8_nl_attention(d8_c)
        d7 = self.dec7(d8_c)
        d7_c = self.dec7_nl(torch.cat((d7, e7), dim=1))
        d7_c, att_7 = self.dec7_nl_attention(d7_c)
        d6 = self.dec6(d7_c)
        d6_c = self.dec6_nl(torch.cat((d6, e6), dim=1))
        d6_c, att_6 = self.dec6_nl_attention(d6_c)
        d5 = self.dec5(d6_c)
        ...
        if out.shape[-1] != self.input_length:
            out = nn.functional.pad(out, (0, abs(self.input_length - out.shape[-1])))
        return out
    ...
    def get_z_shape(self):
        if self.kernel_size / 2 - self.padding > 1.:
            ...
            remainder = 0
        return [self.g_enc_depths[-1], self.input_length // (self.upsamp_ratio ** len(self.g_enc_depths)) - remainder]


class Discriminator(nn.Module):
    """D"""

    def __init__(self, kernel_size=31, downsample_ratio=2):
        super().__init__()
        ...
        negative_slope = 0.03
        self.conv1 = spectral_norm(nn.Conv1d(in_channels=2, out_channels=32, kernel_size=self.kernel_size,
        ...
        self.conv2 = spectral_norm(
            nn.Conv1d(32, 64, self.kernel_size, self.downsample_ratio, self.padding))  # [B x 64 x 4096]
        self.vbn2 = VirtualBatchNorm1d(64)
        self.lrelu2 = nn.LeakyReLU(negative_slope)
        self.conv3 = spectral_norm(
            nn.Conv1d(64, 64, self.kernel_size, self.downsample_ratio, self.padding))  # [B x 64 x 2048]
        self.dropout1 = nn.Dropout()
        self.vbn3 = VirtualBatchNorm1d(64)
        self.lrelu3 = nn.LeakyReLU(negative_slope)
        self.conv4 = spectral_norm(
            nn.Conv1d(64, 128, self.kernel_size, self.downsample_ratio, self.padding))  # [B x 128 x 1024]
        self.vbn4 = VirtualBatchNorm1d(128)
        self.lrelu4 = nn.LeakyReLU(negative_slope)
        self.conv5 = spectral_norm(
            nn.Conv1d(128, 128, self.kernel_size, self.downsample_ratio, self.padding))  # [B x 128 x 512]
        self.vbn5 = VirtualBatchNorm1d(128)
        self.lrelu5 = nn.LeakyReLU(negative_slope)
        self.conv6 = spectral_norm(
            nn.Conv1d(128, 256, self.kernel_size, self.downsample_ratio, self.padding))  # [B x 256 x 256]
        self.dropout2 = nn.Dropout()
        self.vbn6 = VirtualBatchNorm1d(256)
        self.lrelu6 = nn.LeakyReLU(negative_slope)
        self.conv7 = spectral_norm(
            nn.Conv1d(256, 256, self.kernel_size, self.downsample_ratio, self.padding))  # [B x 256 x 128]
        self.vbn7 = VirtualBatchNorm1d(256)
        self.lrelu7 = nn.LeakyReLU(negative_slope)
        self.conv8 = spectral_norm(
            nn.Conv1d(256, 512, self.kernel_size, self.downsample_ratio, self.padding))  # [B x 512 x 64]
        self.vbn8 = VirtualBatchNorm1d(512)
        self.lrelu8 = nn.LeakyReLU(negative_slope)
        self.conv9 = spectral_norm(
            nn.Conv1d(512, 512, self.kernel_size, self.downsample_ratio, self.padding))  # [B x 512 x 32]
        self.dropout3 = nn.Dropout()
        self.vbn9 = VirtualBatchNorm1d(512)
        self.lrelu9 = nn.LeakyReLU(negative_slope)
        self.conv10 = spectral_norm(
            nn.Conv1d(512, 1024, self.kernel_size, self.downsample_ratio, self.padding))  # [B x 1024 x 16]
        self.vbn10 = VirtualBatchNorm1d(1024)
        self.lrelu10 = nn.LeakyReLU(negative_slope)
        self.conv11 = spectral_norm(
            nn.Conv1d(1024, 2048, self.kernel_size, self.downsample_ratio, self.padding))  # [B x 2048 x 8]
        ...
        self.conv_final = spectral_norm(nn.Conv1d(2048, 1, kernel_size=1, stride=1))  # [B x 1 x 8]
...
def print_summary(print_disc=True, print_gen=True, kernel_size=31, stride=2):
    ...
    if print_gen:
        G = Generator(kernel_size=kernel_size, upsamp_ratio=stride)
...
print_summary()
# x = torch.randn(1, 32, 8192)
# y, att = Self_Attn(32)(x)
# print(y.shape)
